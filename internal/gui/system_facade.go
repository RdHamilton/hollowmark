...
// These methods manage the mtga-tracker-daemon subprocess lifecycle.
// ============================================================================

// DaemonProcessStatus represents the current state of the daemon subprocess.
type DaemonProcessStatus struct {
	...
	Port      int     `json:"port"`                // Configured daemon port
	Uptime    float64 `json:"uptime,omitempty"`    // Uptime in seconds if running
	LastError string  `json:"lastError,omitempty"` // Last error message if any
}

// StartDaemonProcess starts the daemon subprocess.
// This is called automatically on app startup if auto-start is enabled.
func (s *SystemFacade) StartDaemonProcess(ctx context.Context) error {
	if s.services.DaemonManager == nil {
		return &AppError{Message: "Daemon manager not initialized"}
	}

	if s.services.DaemonManager.IsRunning() {
		return &AppError{Message: "Daemon is already running"}
	}

	log.Println("Starting daemon subprocess...")

	if err := s.services.DaemonManager.Start(); err != nil {
		log.Printf("Failed to start daemon subprocess: %v", err)
		...
		s.eventDispatcher.Dispatch(events.Event{
			...
			Data: map[string]interface{}{
				...
				"details": "Failed to start the daemon subprocess",
			},
			Context: ctx,
		})

		return &AppError{Message: fmt.Sprintf("Failed to start daemon: %v", err)}
	}

	log.Println("Daemon subprocess started successfully")
	...
	s.eventDispatcher.Dispatch(events.Event{
		...
		Data: map[string]interface{}{
			"status":  "running",
			"pid":     s.services.DaemonManager.PID(),
			"port":    s.services.DaemonManager.Port(),
			"message": "Daemon subprocess started",
		},
		Context: ctx,
	})
...
// StopDaemonProcess gracefully stops the daemon subprocess.
func (s *SystemFacade) StopDaemonProcess(ctx context.Context) error {
	if s.services.DaemonManager == nil {
		return nil // No daemon manager, nothing to stop
	}
	...
	log.Println("Stopping daemon subprocess...")

	// Disconnect IPC client first
	s.stopDaemonClient(ctx)

	// Stop the subprocess
	if err := s.services.DaemonManager.Stop(); err != nil {
		log.Printf("Error stopping daemon subprocess: %v", err)
		return &AppError{Message: fmt.Sprintf("Failed to stop daemon: %v", err)}
	}

	log.Println("Daemon subprocess stopped")
	...
	s.eventDispatcher.Dispatch(events.Event{
		...
		Data: map[string]interface{}{
			"status":  "stopped",
			"message": "Daemon subprocess stopped",
		},
		Context: ctx,
	})
...
func (s *SystemFacade) RestartDaemonProcess(ctx context.Context) error {
	if s.services.DaemonManager == nil {
		return &AppError{Message: "Daemon manager not initialized"}
	}

	log.Println("Restarting daemon subprocess...")

	// Disconnect IPC client
	s.stopDaemonClient(ctx)

	// Restart subprocess
	if err := s.services.DaemonManager.Restart(); err != nil {
		log.Printf("Failed to restart daemon subprocess: %v", err)
		return &AppError{Message: fmt.Sprintf("Failed to restart daemon: %v", err)}
	}

	log.Println("Daemon subprocess restarted successfully")

	// Try to reconnect IPC client after restart
	go func() {
		// Wait for daemon to become ready
		time.Sleep(2 * time.Second)
		if err := s.connectToDaemon(ctx); err != nil {
			log.Printf("Failed to reconnect to daemon after restart: %v", err)
		}
	}()
...
// GetDaemonProcessStatus returns the current status of the daemon subprocess.
func (s *SystemFacade) GetDaemonProcessStatus() *DaemonProcessStatus {
	if s.services.DaemonManager == nil {
	...
	info := s.services.DaemonManager.Info()
	return &DaemonProcessStatus{
...
// SetDaemonAutoStart enables or disables daemon auto-start on app launch.
func (s *SystemFacade) SetDaemonAutoStart(enabled bool) {
	s.services.DaemonAutoStart = enabled
	log.Printf("Daemon auto-start set to: %v", enabled)
}

// IsDaemonAutoStartEnabled returns whether daemon auto-start is enabled.
func (s *SystemFacade) IsDaemonAutoStartEnabled() bool {
	return s.services.DaemonAutoStart
}

// AutoStartDaemon starts the daemon if auto-start is enabled.
// This should be called during app startup.
func (s *SystemFacade) AutoStartDaemon(ctx context.Context) error {
	if !s.services.DaemonAutoStart {
		log.Println("Daemon auto-start is disabled, skipping")
		return nil
	}

	if s.services.DaemonManager == nil {
		log.Println("Daemon manager not initialized, cannot auto-start")
		return nil
	}

	log.Println("Auto-starting daemon subprocess...")
	return s.StartDaemonProcess(ctx)
}

...
|16 | Audit doc PR — land `docs/product/milestones/v0.3.1/daemon-local-api-phase2-audit.md` on main with status header pointing at the live tracker | — | ✅ **Merged** 2026-05-12 | PR #1888 |
|17a | Restore deleted draft algorithm package — grading subpackage of `pkg/draftalgo/` + foundation (types.go, CardLookup / RatingsLookup interfaces) | — | ✅ **Merged** 2026-05-12 | PR #1889 |
|17a2 | Restore remaining draft algorithm packages — `pkg/draftalgo/pickquality/` + `pkg/draftalgo/prediction/` (predictor + synergy). Pure Go pkg, ~900 LOC, unit tests, no daemon wiring | — | ⏳ **In review** | PR #1892 |
|17b | Wire daemon live-state endpoints: `GET /drafts/{id}/current-pack`, `POST /drafts/grade-pick`, `POST /drafts/win-probability`. New `services/daemon/internal/draftstate/` subsystem subscribes to the daemon's LogEntry stream (via the existing logreader, not pkg/logparse — the daemon parses its own log entries), maintains in-memory `map[sessionID]*Session`, exposes a small read interface to the localapi handlers. Handlers call `pkg/draftalgo/pickquality.Analyze` + `pkg/draftalgo/prediction.PredictWinRate`. Delete the 3 retained BFF stubs from PR #14. | 3 | ⏳ **In progress (stacked on #17a2)** | `feat/phase2-pr17b-daemon-live-draft` |
|18 | Daemon uninstall surface — `POST /api/v1/system/uninstall` on daemon localapi; in-process platform dispatch (launchctl unload + plist removal on macOS, schtasks End+Delete on Windows); `?purge=true` flag wipes config; SPA Settings → Danger Zone button | 1 | ✅ **Merged** 2026-05-12 | PR #1890 |
|19 | **Remove Wails-era orphan root module** — `internal/`, `cmd/{mtga-companion,log-analyzer,apiserver}`, `examples/`, `benchmarks/`, root `go.mod` + `go.sum` (mattn/go-sqlite3, modernc.org/sqlite, kardianos/service, wails et al.). Drops the 6 legacy `cmd/mtga-companion-*` binary builds from `.github/workflows/release.yml`; updates `scripts/dev.sh` + `README.md` to point at per-service builds. | -109,994 | ⏳ **In review** | PR #1894 |
|20 | **Daemon ratings client** — new `services/daemon/internal/ratingsclient/` package implementing `draftalgo.CardLookup` + `draftalgo.RatingsLookup` against the BFF's `/api/v1/draft-ratings/{set}/{format}`. Production-grade scope: per-(set,format) TTL cache (24h), `golang.org/x/sync/singleflight` for thundering-herd dedup, bounded retry+backoff on 5xx/network errors, context cancellation, `SetToken` for JWT rotation, `Stats()` for observability, 404→cache-empty (no hammer), `X-Cache-Degraded` header logging, URL escaping, no stale-after-TTL serving. Wired into daemon `Service` via `localapi.SetDraftLookups`. Turns grade-pick / win-probability from "N/A"/baseline stubs into real grades. | — | ⏳ **In progress** | `feat/phase2-pr20-daemon-ratings-client` |

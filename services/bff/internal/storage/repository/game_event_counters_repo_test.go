...
func TestGameEventCountersRepository_InsertAndCount(t *testing.T) {
	...
	gamePlayID, err := gpRepo.InsertGamePlay(ctx, repository.GamePlayInsert{
		AccountID:  accountID,
	...
	t.Cleanup(func() { _, _ = db.ExecContext(ctx, `DELETE FROM game_plays WHERE id = $1`, gamePlayID) })

	inserts := []repository.GameEventCounterInsert{
		{GamePlayID: gamePlayID, AccountID: accountID, InstanceID: 101, ArenaID: 80001, CounterType: "loyalty", Count: 4, Delta: -1, Controller: "player", TurnNumber: 3},
		{GamePlayID: gamePlayID, AccountID: accountID, InstanceID: 102, ArenaID: 80002, CounterType: "+1/+1", Count: 2, Delta: 1, Controller: "opponent", TurnNumber: 5},
	}
	...
	n, err := ctrRepo.CountByGamePlay(ctx, gamePlayID)
	if err != nil {
		t.Fatalf("CountByGamePlay: %v", err)
	}
	if n != 2 {
...
func TestGameEventCountersRepository_OnConflictDoNothing(t *testing.T) {
	...
	ins := repository.GameEventCounterInsert{
		GamePlayID: gamePlayID, AccountID: accountID,
...
func TestGameEventCountersRepository_AccountIsolation(t *testing.T) {
	...
	gpA, err := gpRepo.InsertGamePlay(ctx, repository.GamePlayInsert{AccountID: acctA, MatchID: "match-iso-ctr-a", GameNumber: 1, Sequence: 1, OccurredAt: time.Now().UTC()})
	if err != nil {
		t.Fatalf("InsertGamePlay A: %v", err)
	}
	t.Cleanup(func() { _, _ = db.ExecContext(ctx, `DELETE FROM game_plays WHERE id = $1`, gpA) })

	gpB, err := gpRepo.InsertGamePlay(ctx, repository.GamePlayInsert{AccountID: acctB, MatchID: "match-iso-ctr-b", GameNumber: 1, Sequence: 1, OccurredAt: time.Now().UTC()})
	if err != nil {
		t.Fatalf("InsertGamePlay B: %v", err)
	}
	t.Cleanup(func() { _, _ = db.ExecContext(ctx, `DELETE FROM game_plays WHERE id = $1`, gpB) })

	if err := ctrRepo.InsertCounters(ctx, []repository.GameEventCounterInsert{
		{GamePlayID: gpA, AccountID: acctA, InstanceID: 301, ArenaID: 70001, CounterType: "loyalty", Count: 3, Delta: -2, Controller: "player", TurnNumber: 2},
	}); err != nil {
		t.Fatalf("InsertCounters A: %v", err)
	}

	nA, err := ctrRepo.CountByGamePlay(ctx, gpA)
	if err != nil {
		t.Fatalf("CountByGamePlay A: %v", err)
	}
	nB, err := ctrRepo.CountByGamePlay(ctx, gpB)
	if err != nil {
		t.Fatalf("CountByGamePlay B: %v", err)
	}

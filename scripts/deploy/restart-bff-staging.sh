...
#   (read via /usr/local/bin/${BFF_STAGING_BINARY} --print-embedded-version)
#   against the database's current schema version (schema_migrations MAX(version)).
#
#   stage-binary-staging.sh (deploy step 11) has already mv'd the new binary
#   to /usr/local/bin/${BFF_STAGING_BINARY} before this script runs, so the
#   staged binary is already on disk.  Reading from the staged binary (not from
#   the running service's /healthz) is the correct comparison: we are about to
#   start the staged binary, and we need to know whether it is compatible with
#   the DB.
#
...
if [ "${FORCE_RESTART:-0}" = "1" ]; then
    ...
    STAGED_BINARY="/usr/local/bin/${BFF_STAGING_BINARY}"
    ...
    # stage-binary-staging.sh (deploy step 11) has already placed the new
    # binary at the canonical install path before this script runs.
    BINARY_VERSION=""
    ...
    if [ -z "$BINARY_VERSION" ] || [ "$BINARY_VERSION" = "unknown" ]; then
        echo "[restart-bff-staging] WARN: could not read embedded version from $STAGED_BINARY -- skipping guard (fail-open)."
    else
        ...
        if [ -z "$DB_VERSION" ]; then
            ...
            printf '[restart-bff-staging] ERROR: database schema version (%s) is ahead of staged binary embedded version (%s).\n' \
                "$DB_VERSION" "$BINARY_VERSION" >&2
            printf '[restart-bff-staging] The staged binary does not include migration %s.\n' "$DB_VERSION" >&2
            ...
            printf '[restart-bff-staging] Migration-skew guard PASS: staged-binary=%s db=%s.\n' "$BINARY_VERSION" "$DB_VERSION"
        fi
    fi
fi
